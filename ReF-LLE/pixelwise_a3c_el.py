...
##########################
        ...
        action = np.concatenate((action_r[:,np.newaxis,:,:],action_g[:,np.newaxis,:,:],action_b[:,np.newaxis,:,:]), axis=1)
...
#########################
            ...
            pout_r,pout_g,pout_b, v = self.model.pi_and_v(statevar)
            if self.act_deterministically:
                ...
                return np.concatenate((a_r[:,np.newaxis,:,:],a_g[:,np.newaxis,:,:],a_b[:,np.newaxis,:,:]), axis=1),v
            else:
